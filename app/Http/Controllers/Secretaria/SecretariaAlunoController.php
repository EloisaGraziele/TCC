...
class SecretariaAlunoController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            // 3️⃣ Geração do QR Code com dados criptografados
            $qrCodeValue = encrypt($aluno->nome . '|' . $aluno->matricula . '|' . $aluno->turma->turma); // Criptografando os dados
            $qrCodeSvg = QrCode::size(150)
                ->errorCorrection('H')
                ->format('svg')
                ->margin(1)
                ->generate($qrCodeValue); // Gerando o QR Code
    ...
    {
        // 1️⃣ Gerando o QR Code com os dados criptografados
        $qrCodeValue = encrypt($aluno->nome . '|' . $aluno->matricula . '|' . $aluno->turma->turma); // Criptografando os dados
